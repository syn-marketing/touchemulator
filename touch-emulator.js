(function(window, document, exportName, undefined) {
    ...
    function TouchEmulatorFactory(win = window, doc = document) {
        var isRunning = false;
        ...
        if(!doc.createTouch) {
            doc.createTouch = function(view, target, identifier, pageX, pageY, screenX, screenY, clientX, clientY) {
                ...
                if(clientX == undefined || clientY == undefined) {
                    clientX = pageX - win.pageXOffset;
                    clientY = pageY - win.pageYOffset;
                }
        ...
        function fakeTouchSupport() {
            var objs = [win, doc.documentElement];
        ...
        function hasTouchSupport() {
            return ("ontouchstart" in win) || // touch events
                   (win.Modernizr && win.Modernizr.touch) || // modernizr
        ...
        function onMouse(touchType) {
            return function(ev) {
                if (TouchEmulatorFunction.ignoreTags.indexOf(ev.target.tagName) < 0) {
        ...
        function triggerTouch(eventName, mouseEv) {
            var touchEvent = doc.createEvent('Event');
        ...
        function createTouchList(mouseEv) {
            ...
            if (isMultiTouch) {
                var f = TouchEmulatorFunction.multiTouchOffset;
        ...
        function showTouches(ev) {
            ...
            for(i = 0; i < ev.touches.length; i++) {
                ...
                if(!el) {
                    el = touchElements[touch.identifier] = doc.createElement("div");
                    doc.body.appendChild(el);
                }

                styles = TouchEmulatorFunction.template(touch);
        ...
        function removeHandlers() {
            for(var i=0; i<handlers.length; i++) {
                win.removeEventListener(handlers[i].event, handlers[i].handler, true);
            }
        ...
        /**
         * TouchEmulatorFunction initializer
         */
        function TouchEmulatorFunction() {
            TouchEmulatorFunction.start();
        }

        TouchEmulatorFunction.start = function () {
        ...
        TouchEmulatorFunction.stop = function () {
        ...
        TouchEmulatorFunction.multiTouchOffset = 75;

        // tags that shouldn't swallow mouse events
        TouchEmulatorFunction.ignoreTags = ['TEXTAREA', 'INPUT', 'SELECT'];
         ...
         * @returns object
         */
        TouchEmulatorFunction.template = function(touch) {
            var size = 30;
        ...
        return TouchEmulatorFunction;
    }

    var TouchEmulator = TouchEmulatorFactory();
    TouchEmulator.factory = TouchEmulatorFactory;
